...
def computeFixedMetricAllForecasts(forecasts, realCrimes, date_idx, clusters, cell_coverage_units=1, available_resources=10000):
    ...
    available, allocation = assignResources(forecast, clusters, cell_coverage_units, available_resources)
    ...
    potential = pd.Series([ allocation[i] * cell_coverage_units // clusters.Area.values[i] for i in range(len(clusters.Area.values))])
    # print("potential", potential)
    # Result: real crimes avoided
    # print(np.array(realCrimes).shape)
    # print(date_idx)
    realCrimeIdx = len(realCrimes) - len(realCrimes) // 3 + date_idx 
    avoided = pd.DataFrame([realCrimes.iloc[realCrimeIdx], potential]).min().map(lambda r: 0 if r<0 else r)
...
def computeFullMetricAllForecasts(forecasts, realCrimes, date_idx, clusters, cell_coverage_units=1):
    ...
    _, allocation = assignResources(forecast, clusters, cell_coverage_units, available_resources=np.inf)
    ...
    potential = pd.Series(forecast.values, index=[f[:-len('Forecast')]+'Crimes' for f in forecast.index])
    # Result: real crimes avoided
...
def computeStoppedCrime(ignoreFirst, forecasts, realCrimes, clusters, cell_coverage_units=1, available_resources=10000):
    ...
    return pd.Series(data=[
        computeFixedMetricAllForecasts(forecasts, realCrimes, date_idx, clusters, cell_coverage_units, available_resources).sum()
...
def fixResourceAvailable(resource_indexes, ignoreFirst, forecasts, realCrimes, clusters, cell_coverage_units, grid_lat, grid_lon):
    ...
    clusters['Area'] = clusters.Geometry.map(lambda g: g.data.size * 400 / (grid_lat * grid_lon))
    clusters.sort_values(by=['Area','Crimes'], ascending=[True, False], inplace=True)
    
    # compute for all amount of available resources
    return pd.Series(data=[
        computeRelativeStoppedCrime(0, forecasts, realCrimes, clusters, cell_coverage_units, available_resources=available).mean()
...
def fullMetric_(prop_indexes, ignoreFirst, forecasts, realCrimes, clusters, cell_coverage_units):
    ...
    return computeFullEfficiency(0, forecasts, realCrimes, clusters, cell_coverage_units)#.mean()

def fullMetric(ignoreFirst, forecasts, realCrimes, clusters, cell_coverage_units, A_coverage_units):
    clusters['Area'] = clusters.Geometry.map(lambda g: g.data.size)
    
    _error = (forecasts.applymap(lambda r: 0 if r<0 else r).values - realCrimes.values)[ignoreFirst:]
    errors = pd.DataFrame(data=_error,
                          columns=[c[:-len('Forecast')]+'Error' for c in forecasts.columns],
