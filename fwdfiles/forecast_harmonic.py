...
def init_predictions_Harmonic():
    ...
    #     fDyHarm <- function(y, h) {
    #         Kvalues <- c(1,11)
    #         model <- auto.arima(y, xreg=fourier(y, K=Kvalues), seasonal=FALSE)
    #         fc <- forecast(model, xreg=fourier(y, K=Kvalues, h=h))
    #     }
    ...
    #         log_name <- paste('parallel_',sample(1:100000, 1),'.log', sep='')
    #         writeLines(c(''), log_name)
    ...
    #             .export=c('computeClusterErrors', 'fDyHarm')) %dopar% {
    #             sink(log_name, append=TRUE)
    #             cat(paste('computing cluster', cluster,'...\n'))
    #             sink()
    #             computeClusterErrors(realCrimes[cluster], initialYear, initialWeek, periodsAhead, fDyHarm)
    #         }
    #         stopCluster(cl)
    #         ret
    #     }
    ...
    for c in clusters.Cluster.values:
        ...
        if (periodsAhead == 1) or len(orders) == 1:
            stepwise_model = auto_arima(train, start_p=0, start_q=0,
                                        max_q=5, max_p=5, m=52,
                                        start_P=0, max_P=5, start_Q=0, max_Q=5, seasonal=True,
                                        trace=True,
                                        error_action='ignore',
