...
def computeClustersAndOrganizeData(ts, gridshape=(60, 85), ignoreFirst=149, periodsAhead=52, threshold=4000, maxDist=5):
    ...
    ts = pd.DataFrame({'Crimes': newDataGrid.groupby(
...
def main():
    ...
    data.drop(data[data.Timestamp < 693596].index,
              inplace=True)  # 693596: 1/1/1900
    ...
    clusters, realCrimes = computeClustersAndOrganizeData(
        data, conf.gridshape, conf.ignoreFirst, conf.periodsAhead, conf.threshold, conf.maxDist)

    print('Computing predictions ...')
    forecasts = None
    if (conf.args.f_harmonic):
        forecast_harmonic.init_predictions_Harmonic()
        forecasts = forecast_harmonic.predictions_Harmonic(
            clusters, realCrimes, periodsAhead=conf.periodsAhead)
    elif (conf.args.f_autoregressive):
        forecast_ar.init_predictions_AR()
        forecasts = forecast_ar.predictions_AR(
            clusters, realCrimes, periodsAhead=conf.periodsAhead)
    else:
        forecast_mean.init_predictions_Mean(window=conf.f_mm_window)
        forecasts = forecast_mean.predictions_Mean(
            clusters, realCrimes, periodsAhead=conf.periodsAhead)
    savePredictions(clusters, realCrimes, forecasts, conf.gridshape,
                    conf.ignoreFirst, conf.periodsAhead, conf.threshold, conf.maxDist)
